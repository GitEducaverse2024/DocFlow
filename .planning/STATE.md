...
- **Plan 137-01 (INC-11/INC-12/INC-13):** Cierre de los tres bugs de runtime descubiertos en Phase 136 gate, sin tocar canvas-executor.ts ni execute-catpaw.ts. INC-11: render_template en catpaw-email-template-executor.ts ahora extrae required keys de cada instruction block.text de header/body/footer (template-renderer mapea variables[block.text] — ése es el contrato real, no structure.variables[]); doble defensa con detectUnresolvedPlaceholders buscando {{X}} residual Y el literal "Contenido principal del email" observado en Phase 136. INC-12 cerrado a nivel wrapper (Option B sancionado, Option A deferred a v27.2): send_email valida to/subject/body|html_body no vacios + assert messageId mandatorio en respuesta. Si sendEmail() devuelve sin messageId, el wrapper emite {error, raw_response}; el agent LLM recibe error en tool-call history y no puede fabricar success sin hallucination detectable. 137-06 signal-gate agrega linea al acceptance form validando que wrapper errors se propagan al output textual del agent emitter. INC-13: connector_logs en gmail/email-template/drive executors persisten {operation,pawId,args} como request_payload y real tool result como response_payload (no mas {ok:true}). safeStringify aplica cap duro 10_000 chars por payload. redactAndTrimArgs/redactAndTrimDriveArgs reemplazan body/html_body/content por *_len (size control + privacy) y [REDACTED]ean credenciales por key name (access_token, refresh_token, api_key, password, client_secret, authorization, cookie, oauth_token). Failure path de los 3 executors reescrito con la misma shape rica. Rule 3 auto-fix: _index.json canvas updated_at bumped a 2026-04-11 para satisfacer knowledge-tree parity test tras el update de canvas.json. Knowledge tree per CLAUDE.md: canvas.json + catflow.json documentan INC-11/INC-12 como common_errors con cause+solution; catflow.json howto agrega entrada de diagnostico post-mortem via connector_logs rico. .planning/knowledge/connector-logs-redaction-policy.md creado con tabla por operacion, campos redactados, debug mode TODO, ejemplos before/after, VALIDATION-05 playbook. TDD estricto 4 commits atomicos (014342b RED1 4 failed / 184b7ba GREEN1 5/5 / 946f780 RED2 8 failed / 1a79601 GREEN2 11/11 + parity fix). 35/35 tests verdes finales en las tres suites tocadas. Failures pre-existentes en catbot-prompt-assembler + task-scheduler + catbot-holded-tools verificadas out-of-scope via git stash — pertenecen a workflows 137-02/137-03 en vuelo en el mismo working tree.
