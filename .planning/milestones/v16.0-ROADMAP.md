# Roadmap: DoCatFlow v16.0 — CatFlow

**Milestone:** v16.0
**Shipped:** 2026-03-22
**Phases:** 8 (Phase 63-70)
**Requirements:** 76 total, 76 mapped (100%)
**Stats:** 182 commits, 193 files changed, +21K/-4K lines

## Key Accomplishments

1. **Rename Tareas → CatFlow**: Sidebar, routes, i18n, breadcrumbs — backward compatible /tasks
2. **3 new canvas nodes**: Scheduler (delay/count/listen), Storage (local/connector/LLM format), MultiAgent (sync/async inter-CatFlow)
3. **Config panel redesign**: Right sidebar w-80 replacing bottom panel, with copy/paste (Ctrl+C/V)
4. **Inter-CatFlow communication**: catflow_triggers table, API endpoints, START listen_mode + OUTPUT trigger chains
5. **Enhanced START + OUTPUT**: Listen mode badge/handle, notifications on complete, fire-and-forget trigger chain
6. **CatBot tools + tests**: 4 new tools (list/execute/toggle/fork), 8 E2E + 3 API specs, i18n audit
...
- [x] **Phase 65: Scheduler Node** - Canvas node with delay/count/listen modes, multi-handle routing, signal endpoint
...
- **69/76 PASS**, 5 PARTIAL, 2 FAIL (cosmetic: PAGE-08 dropdown, PAGE-09 collapsible section)
- **1 critical bug found and fixed**: BOT-03 — catbot-tools.ts queried `tasks` table instead of `canvases` table for toggle/list operations
- Requirements bookkeeping gap resolved during archival (checkboxes updated)
