...
## Prompt 3 — KB-42 Link Tools + Template ⚠ PARTIAL
...
**Propuesta de cierre:** Extender `buildFrontmatter` para popular `search_hints` con los nombres de conectores y skills linked. Cambio scoped a `knowledge-sync.ts` (≤15 líneas). Dejarlo para Phase 156.1 (gap-closure decimal) o aceptar como refinamiento del criterio KB-42 (el body refleja lo correcto; el índice es afinable). **Decisión delegada al verifier.**

**Verdict:** ⚠ Template side fully verified (KB-42 Gap a closed). Index-level search_kb match is the open refinement.
...
| 3 | KB-42 link + template | ⚠ PARTIAL | Template + hook 100%; search_kb por body-text requiere extension de `search_hints` (decisión verifier) |
| 4 | KB-43 counts reconciliation | ✓ PASSED | 5/6 entidades; +1 email-templates KB-44 orthogonal |

**Score oracle:** 3.5 / 4 prompts fully passed. 1 partial con gap concreto y propuesta de cierre.
