...
import { Loader2, ClipboardList, Plus, Clock, Bot, FolderKanban, Trash2, Zap } from 'lucide-react';
...
export function TaskListContent() {
  ...
  const isCatFlow = pathname.startsWith('/catflow');
  const basePath = isCatFlow ? '/catflow' : '/tasks';
  ...
  // Header icon and title based on route
  const headerIcon = isCatFlow
    ? <Zap className="w-6 h-6" />
    : <ClipboardList className="w-6 h-6" />;
  const headerTitle = isCatFlow ? 'CatFlow' : t('title');
  const headerDescription = isCatFlow ? t('description') : t('description');
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => router.push(`${basePath}/new?template=${tmpl.id}`)}
