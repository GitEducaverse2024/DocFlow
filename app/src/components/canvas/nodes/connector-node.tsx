    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-sm font-semibold text-orange-100 truncate">
          {nodeData.label || t('nodes.connector')}
        </span>
      </div>
      <div className="text-xs text-zinc-400 truncate">
        {nodeData.connectorName || (nodeData.connectorId ? nodeData.connectorId : t('nodes.noConnector'))}
      </div>
      {nodeData.mode && (
        <span className={`mt-1 inline-block text-[10px] px-1.5 py-0.5 rounded-full ${
          ...
          {nodeData.mode === 'before' ? t('nodes.before') : t('nodes.after')}
        </span>
      )}
      <Handle
        type="source"
