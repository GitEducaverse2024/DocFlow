    ...
    return (
      <div className="rounded-lg p-4 border bg-emerald-500/10 border-emerald-500/20">
        <div className="flex items-center gap-2 text-emerald-400 text-sm font-medium">
          ...
          {result.message || 'Agente creado y activado'}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-lg p-4 space-y-3 border bg-violet-500/10 border-violet-500/20">
        <div className="flex items-center gap-2 text-violet-400 text-sm font-medium">
          ...
          {countdown > 0
            ? `Agente creado. El gateway se reiniciará en ~${countdown}s...`
            : 'Agente creado. El gateway debería haberse reiniciado.'
          }
        </div>
        {result.restartCommand && (
          <div className="flex items-center gap-2">
            <p className="text-[10px] text-zinc-500 flex-shrink-0">Si no se activa:</p>
  ...
  return (
    <div className="rounded-lg p-4 space-y-2 border bg-amber-500/10 border-amber-500/20">
      <div className="flex items-center gap-2 text-amber-400 text-sm font-medium">
        ...
        {result.warning || result.message || 'Agente creado'}
      </div>
    </div>
  );
}
...
export function AgentCreator({
  ...
  // Common fields
  const [agentName, setAgentName] = useState(projectName ? `Experto en ${projectName}` : '');
  ...
  const handleGenerate = async () => {
    if (!agentModel) { toast.error('Selecciona un modelo primero'); return; }
    setGenerating(true);
    setGenerated(null);
    setEditing(false);
    try {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (created.status === 'active') {
        toast.success('Agente creado y activado', { description: created.message });
      } else if (created.warning) {
        toast.success('Agente creado', { description: created.warning, duration: 8000 });
      } else {
  ...
  const modeCards: { key: CreationMode; icon: typeof PenLine; label: string; desc: string }[] = [
    { key: 'manual', icon: PenLine, label: 'Manual', desc: 'Rellena los datos básicos' },
    { key: 'from-skill', icon: Zap, label: 'Desde Skill', desc: 'Usa una skill como base' },
    { key: 'ai-generate', icon: Sparkles, label: 'Generar con IA', desc: 'El LLM genera todo' },
  ];
    ...
    <div>
      ...
      {loadingModels ? (
        <div className="flex items-center gap-2 text-xs text-zinc-500 py-2">
          <Loader2 className="w-3 h-3 animate-spin" /> Cargando modelos...
        </div>
      ) : hasManagedModels ? (
    ...
    return (
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4 space-y-3">
        ...
        <div className="bg-zinc-950 rounded p-3">
          <p className="text-xs text-zinc-500 mb-1">Personalidad (SOUL.md):</p>
        ...
        {editing && (
          <div className="space-y-3">
            <div>
              <Label className="text-xs text-zinc-500 mb-1 block">SOUL.md — Personalidad</Label>
              <Textarea
                value={editSoul}
            ...
            <div>
              <Label className="text-xs text-zinc-500 mb-1 block">AGENTS.md — Instrucciones operativas</Label>
              <Textarea
                value={editAgents}
            ...
            <div>
              <Label className="text-xs text-zinc-500 mb-1 block">IDENTITY.md — Identidad</Label>
              <Textarea
                value={editIdentity}
  ...
  return (
    <div className="space-y-4">
      ...
      {mode === 'manual' && !creationResult && (
        <div className="space-y-3 bg-zinc-900/30 border border-zinc-800 rounded-lg p-4">
          <div className="grid grid-cols-[60px_1fr] gap-3">
            ...
            <div>
              <Label className="text-xs text-zinc-400 mb-1 block">Nombre del agente</Label>
              <Input
                value={agentName}
          ...
          <div>
            <Label className="text-xs text-zinc-400 mb-1 block">Descripción / Especialidad</Label>
            <Textarea
              value={agentDesc}
              onChange={(e) => setAgentDesc(e.target.value)}
              placeholder="Qué debe hacer este agente..."
          ...
          <p className="text-[10px] text-zinc-500">
            Se crearán archivos básicos en español. Una IA los mejorará automáticamente después de crear.
          </p>
      ...
      {mode === 'from-skill' && !creationResult && (
        <div className="space-y-3 bg-zinc-900/30 border border-zinc-800 rounded-lg p-4">
          <div>
            <Label className="text-xs text-zinc-400 mb-1 block">Selecciona un skill como base</Label>
            {loadingSkills ? (
              <div className="flex items-center gap-2 text-xs text-zinc-500 py-2">
                <Loader2 className="w-3 h-3 animate-spin" /> Cargando skills...
              </div>
            ) : skills.length === 0 ? (
              <p className="text-xs text-zinc-500 py-2">No hay skills disponibles. Crea uno primero en /skills.</p>
            ) : (
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (skill.id !== selectedSkillId) {
                        if (!agentName || agentName === `Experto en ${projectName}`) setAgentName(skill.name);
          ...
          {selectedSkill && (
            <>
              ...
              >
                ...
                {generating ? 'Generando desde skill...' : 'Generar configuración desde skill'}
              </Button>
            </>
          )}
            ...
            <>
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <Button variant="outline" size="sm" onClick={() => setEditing(!editing)}
                  className="bg-transparent border-zinc-700 text-zinc-300 hover:bg-zinc-800">
                  <Pencil className="w-3.5 h-3.5 mr-1.5" /> {editing ? 'Ocultar editor' : 'Editar'}
                </Button>
        ...
        <div className="space-y-3 bg-zinc-900/30 border border-zinc-800 rounded-lg p-4">
          <div className="grid grid-cols-[60px_1fr] gap-3">
            ...
            <div>
              ...
              <Input
                value={agentName}
                onChange={(e) => setAgentName(e.target.value)}
                placeholder={`Experto en ${projectName || 'tu proyecto'}`}
                className="bg-zinc-950 border-zinc-800 text-zinc-50"
              />
            </div>
          </div>
          <div>
            <Label className="text-xs text-zinc-400 mb-1 block">Especialidad / Descripción</Label>
            <Textarea
              value={agentDesc}
              onChange={(e) => setAgentDesc(e.target.value)}
              placeholder="Qué debe hacer este agente..."
          ...
          {!generated && !generating && (
            ...
            >
              ...
              Crear sin generar (datos mínimos)
            </Button>
          )}
        </div>
      )}
