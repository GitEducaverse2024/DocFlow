...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const project = db.prepare('SELECT * FROM catbrains WHERE id = ?').get(projectId) as { current_version: number, agent_id: string, name: string, purpose: string, tech_stack: string };
    if (!project) {
    ...
    if (activeRun) {
      return NextResponse.json({ error: 'A process is already running for this CatBrain' }, { status: 409 });
    }
    ...
    db.prepare(`UPDATE catbrains SET current_version = ?, status = 'processing', updated_at = ? WHERE id = ?`).run(newVersion, new Date().toISOString(), projectId);
    ...
    const webhookPayload = {
      ...
      callback_url: `http://${process['env']['SERVER_HOSTNAME'] || 'localhost'}:${process['env']['PORT'] || 3500}/api/catbrains/${projectId}/process/callback`,
    ...
    const startLocalProcessing = async () => {
      try {
        ...
        db.prepare(`UPDATE catbrains SET status = 'processed' WHERE id = ?`).run(projectId);

        createNotification({
          type: 'process',
          title: `Procesamiento completado`,
          message: `CatBrain procesado exitosamente (v${newVersion})`,
          severity: 'success',
          link: `/catbrains/${projectId}`,
        });
        ...
        createNotification({
          ...
          message: `Error procesando CatBrain: ${(error as Error).message}`.slice(0, 200),
          severity: 'error',
          link: `/catbrains/${projectId}`,
        });
      }
