...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string; vid: string }> }) {
  try {
    ...
    const version = parseInt(vid, 10);

    if (isNaN(version)) {
      return NextResponse.json({ error: 'Invalid version' }, { status: 400 });
    }

    const project = db.prepare('SELECT * FROM catbrains WHERE id = ?').get(projectId) as { current_version: number } | undefined;
    if (!project) {
    ...
    if (project.current_version === version) {
      ...
      db.prepare(`UPDATE catbrains SET current_version = ?, updated_at = ? WHERE id = ?`).run(newVersion, new Date().toISOString(), projectId);
    }
