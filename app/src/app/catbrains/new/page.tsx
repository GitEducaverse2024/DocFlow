...
export default function NewCatBrain() {
  ...
  const t = useTranslations('catbrains.new');
  ...
  const handleCreateDraft = async () => {
    try {
      ...
      if (!res.ok) throw new Error(t('errors.createError'));
  ...
  const handleFinish = async () => {
    try {
      ...
      toast.error(t('errors.finishError'));
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8 relative max-w-2xl mx-auto">
        ...
        {[
          { num: 1, label: t('steps.information') },
          { num: 2, label: t('steps.sources') },
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardHeader>
          <CardTitle className="text-xl text-zinc-50">
              {step === 1 && t('stepTitles.information')}
              {step === 2 && t('stepTitles.sources')}
              {step === 3 && t('stepTitles.agent')}
            </CardTitle>
            <div className="mt-2">
              {step === 1 && <HelpText text={t('help.information')} />}
              {step === 2 && <HelpText text={t('help.sources')} />}
              {step === 3 && <HelpText text={t('help.agent')} />}
            </div>
        </CardHeader>
        <CardContent>
          {step === 1 && (
            <div className="space-y-6">
              <div>
                ...
                <Input
                  ...
                  maxLength={100}
                />
                {errors.name && <p className="text-red-500 text-xs mt-1">{t('errors.nameRequired')}</p>}
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <HelpText text={t('fields.purposeHelp')} />
                  </div>
                <Textarea
                  ...
                  maxLength={1000}
                />
                {errors.purpose && <p className="text-red-500 text-xs mt-1">{t('errors.purposeRequired')}</p>}
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <HelpText text={t('fields.techStackHelp')} />
                  </div>
                <Input
            ...
            <div className="space-y-6">
              {loadingAgents ? (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-zinc-400">{t('agents.loading')}</p>
                </div>
              ) : agentsError || (agents.length === 0 && !isFallback) ? (
                <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-6 text-center">
                  <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
                  <h3 className="text-lg font-medium text-zinc-50 mb-2">{t('agents.noAgents')}</h3>
                    ...
                    <div>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-zinc-50">{t('agents.noAgentNow')}</span>
                        </div>
                        <p className="text-sm text-zinc-400">{t('agents.noAgentDescription')}</p>
                      </Label>
                    </div>
                  </div>
                </RadioGroup>
